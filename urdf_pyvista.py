...
class URDF:
    ...
    def _generate_link_colors(self, num_links):
        ...
        Args:
            num_links (int): Number of distinct colors needed.
    ...
    def _get_link_name_from_geometry_name(self, geom_name, scene):
        """Extract link name from geometry name based on scene graph.
        ...
        Returns:
            str: Name of the parent link or the geometry name if not found.
        """
        # Try to find the link name by traversing the scene graph
        if hasattr(scene, 'graph') and scene.graph is not None:
            try:
                # Get all nodes that have this geometry
                for node_name in scene.graph.nodes:
                    if node_name in scene.graph.nodes_geometry:
                        if scene.graph.nodes_geometry[node_name] == geom_name:
                            return node_name
                    
                # If not found directly, look for parent nodes
                for node_name in scene.graph.nodes:
                    if geom_name.startswith(node_name) or node_name in geom_name:
                        return node_name
            except Exception as e:
                print(f"Error occurred while finding link name: {e}")
                pass
        
        # Fallback: try to extract link name from geometry name
        # Common patterns: "link_name_visual", "link_name_collision", etc.
        parts = geom_name.split('_')
        if len(parts) > 1:
            # Remove common suffixes
            for suffix in ['visual', 'collision', 'mesh', '0', '1', '2', '3', '4', '5']:
                if parts[-1].lower() == suffix:
                    return '_'.join(parts[:-1])
        
        return geom_name

    def _update_visual_meshes(self, plotter, scene, geometry_to_link_map, link_colors, color_by_link):
        """Update the visual meshes in the plotter with current transformations from scene graph."""
        for name, geom in scene.geometry.items():
            ...
            try:
                # Get current transform for this geometry from scene graph
                if hasattr(scene, 'graph') and scene.graph is not None:
                    if name in scene.graph.nodes:
                        # Get world transform for this geometry
                        transform_matrix = scene.graph.get(frame_to=name)[0]
                        ...
                        if hasattr(mesh, 'vertices') and hasattr(mesh, 'faces'):
                            ...
                            # PyVista expects faces to have the number of vertices per face prepended
                            if faces.ndim == 2 and faces.shape[1] == 3:
                                faces_pv = np.column_stack([
                                    np.full(faces.shape[0], 3),
                                    faces
                                ]).flatten()
                            else:
                                faces_pv = faces.flatten() if faces.ndim > 1 else faces
                            
                            # Create updated PyVista mesh
                            pv_mesh = pv.PolyData(transformed_vertices, faces_pv)
                            
                            # Determine color
                            mesh_color = 'lightgray'
                            if color_by_link:
                                if name in geometry_to_link_map:
                                    link_name = geometry_to_link_map[name]
                                    # Make first link grey
                                    if link_name == self.robot.links[0].name:
                                        mesh_color = 'grey'
                                    elif link_name in link_colors:
                                        mesh_color = link_colors[link_name]
                            
                            # Remove old mesh and add updated one
                            if name in plotter.actors:
                                plotter.remove_actor(name)
                            plotter.add_mesh(pv_mesh, name=name, show_edges=False, color=mesh_color)
    ...
    def show(self, collision_geometry=False, notebook=False, window_size=(1024, 768), color_by_link=False, sliders=False, grid=True):
        ...
        Args:
            ...
            callback (function, optional): A callback function to customize the plotter before showing.
        ...
        elif hasattr(scene, 'geometry') and len(scene.geometry) < 1:
            raise ValueError(
                "Scene is empty, maybe meshes failed to load? Use build_scene_graph=True and load_meshes=True during loading."
            )
        
        # Configure PyVista for proper display
        try:
            # Try to set a compatible backend
            if notebook:
                pv.set_jupyter_backend('static')
            else:
                # For desktop applications, use VTK backend
                pv.set_plot_theme('document')
                
        except Exception as e:
            print(f"Warning: Could not set PyVista backend: {e}")
        ...
        if notebook:
            ...
            plotter = pv.Plotter(
                ...
                title="PyVista URDF Model Viewer"
            )
        
        # Set up link coloring if requested
        link_colors = {}
        geometry_to_link_map = {}
        
        if color_by_link:
            # Get actual link names from URDF structure
            link_names = [link.name for link in self.robot.links]
            
            # Generate colors for each link
            colors = self._generate_link_colors(len(link_names))
            link_colors = dict(zip(link_names, colors))
            
            # Create mapping from geometry names to link names using scene graph
            if hasattr(scene, 'graph') and scene.graph is not None:
                # Strategy: find the link with the shortest path to each geometry
                for geom_name in scene.geometry.keys():
                    min_distance = float('inf')
                    best_link = None
                    
                    # Find the link with the shortest path to this geometry
                    for link_name in link_names:
                        if link_name in scene.graph.nodes:
                            try:
                                path = scene.graph.transforms.shortest_path(link_name, geom_name)
                                if path and len(path) < min_distance:
                                    min_distance = len(path)
                                    best_link = link_name
                            except Exception:
                                # If no path exists, skip this link
                                continue
                    
                    if best_link:
                        geometry_to_link_map[geom_name] = best_link
                    else:
                        # Fallback: assign to first available link
                        geometry_to_link_map[geom_name] = link_names[0] if link_names else 'unknown'
            
            print(f"Coloring {len(link_names)} links: {link_names}")
            print(f"Geometry to link mapping: {geometry_to_link_map}")

        # Function to add meshes to the plotter
            ...
            for name, geom in scene.geometry.items():
                ...
                try:
                    ...
                    if hasattr(mesh, 'vertices') and hasattr(mesh, 'faces'):
                        ...
                        # PyVista expects faces to have the number of vertices per face prepended
                        if faces.ndim == 2 and faces.shape[1] == 3:
                            # Triangular faces - prepend 3 to each face
                            faces_pv = np.column_stack([
                                np.full(faces.shape[0], 3),
                                faces
                            ]).flatten()
                        else:
                            # Handle other face formats
                            faces_pv = faces.flatten() if faces.ndim > 1 else faces
                        
                        pv_mesh = pv.PolyData(vertices, faces_pv)
                        
                        # Determine color to use
                        mesh_color = 'lightgray'  # default
                        
                        if color_by_link:
                            # Use link-based coloring
                            if name in geometry_to_link_map:
                                link_name = geometry_to_link_map[name]
                                # Make first link grey
                                if link_name == self.robot.links[0].name:
        ...
        # Add initial meshes
        mesh_count = _add_meshes_to_plotter()
        
        if mesh_count == 0:
            print("Warning: No meshes were successfully added to the scene")
            return
            
        # Set up camera and lighting with Y-up orientation
        plotter.camera_position = 'iso'  # Use 'iso' instead of 'isometric'
        plotter.camera.up = (0, 1, 0)  # Set Y axis as up
        plotter.add_axes()
        ...
        if self.num_actuated_joints > 0 and sliders:
            ...
            slider_spacing = 0.12  # Further increased vertical spacing between sliders
            slider_start_y = 0.88   # Starting Y position (adjusted for larger spacing)
            
            for i, joint in enumerate(self.actuated_joints):
                # Get joint limits
                if joint.limit is not None:
                    lower_limit = joint.limit.lower if joint.limit.lower is not None else -3.14159
                    upper_limit = joint.limit.upper if joint.limit.upper is not None else 3.14159
                else:
                    # Default limits for joints without specified limits
                    if joint.type == "prismatic":
                        lower_limit, upper_limit = -1.0, 1.0
                    else:  # revolute or continuous
                        lower_limit, upper_limit = -3.14159, 3.14159
                
                # Convert revolute joint limits to degrees for display
                is_revolute = joint.type in ["revolute", "continuous"]
                if is_revolute:
                    lower_limit_display = np.degrees(lower_limit)
                    upper_limit_display = np.degrees(upper_limit)
                else:
                ...
                # Callback that updates joint configuration and refreshes visual meshes
                def make_joint_callback(joint_idx, joint_name, joint_type):
                    def callback(value):
                        # Convert from degrees to radians for revolute joints
                        if joint_type in ["revolute", "continuous"]:
                            internal_value = np.radians(value)
                            display_unit = "°"
                        else:
                            internal_value = value
                            display_unit = ""
                        
                        current_joint_values[joint_idx] = internal_value
                        self.update_cfg(current_joint_values)  # Update internal state
                        
                        # Update visual meshes with new transformations
                        try:
                            self._update_visual_meshes(plotter, scene, geometry_to_link_map, link_colors, color_by_link)
                            print(f"  → Visual meshes updated for {joint_name}")
                        except Exception as e:
                            print(f"Warning: Could not update visual meshes: {e}")
                        
                        print(f"{joint_name}: {value:.3f}{display_unit}")
                    return callback
                
                # Create smaller slider widget
                slider_widget = plotter.add_slider_widget(
                    callback=make_joint_callback(i, joint.name, joint.type),
                    rng=[lower_limit_display, upper_limit_display],
                    value=current_value_display,
                    title=f"{joint.name} ({'°' if is_revolute else ''})",  # Add degree symbol for revolute joints
                    pointa=(0.02, slider_y),      # Start position
                    pointb=(0.25, slider_y),      # Slightly longer slider for better visibility
                    style='modern'
                )
                
                # Store slider reference with initial values for reset functionality
                slider_widgets.append({
                    'widget': slider_widget,
                    'joint_idx': i,
                    'joint_name': joint.name,
                    'joint_type': joint.type,
                    'initial_value': current_value_display,
                    'is_revolute': is_revolute
                })
                
                # Print joint info with appropriate units
                unit_str = "° (degrees)" if is_revolute else ""
                print(f"  {i+1}. {joint.name} ({joint.type}): [{lower_limit_display:.1f}, {upper_limit_display:.1f}]{unit_str} = {current_value_display:.1f}{unit_str}")
            
            # Add reset button to initialize all sliders to zero
            def reset_all_sliders(checkbox_value=None):
                print("Resetting all sliders to zero positions...")
                zero_values = [0.0] * self.num_actuated_joints
                
                # Update each slider to zero (or initial position)
                for slider_info in slider_widgets:
                    try:
                        # For revolute joints, reset to 0 degrees, for prismatic to 0 units
                        reset_value = 0.0
                        
                        # Set slider value (this should trigger the callback)
                        if hasattr(slider_info['widget'], 'GetRepresentation'):
                            slider_info['widget'].GetRepresentation().SetValue(reset_value)
                        
                        # Also update internal values directly
                        if slider_info['joint_type'] in ["revolute", "continuous"]:
                            internal_value = np.radians(reset_value)
                        else:
                            internal_value = reset_value
                        
                        zero_values[slider_info['joint_idx']] = internal_value
                        print(f"  Reset {slider_info['joint_name']}: {reset_value:.3f}{'°' if slider_info['is_revolute'] else ''}")
                        
                    except Exception as e:
                        print(f"  Warning: Could not reset slider {slider_info['joint_name']}: {e}")
                
                # Update robot configuration
                self.update_cfg(zero_values)
                
                # Update visual meshes
                try:
                    self._update_visual_meshes(plotter, scene, geometry_to_link_map, link_colors, color_by_link)
                    print("  → Visual meshes updated to reset positions")
                except Exception as e:
                    print(f"Warning: Could not update visual meshes after reset: {e}")
            
            # Add reset button widget
            button_y = slider_start_y - ((self.num_actuated_joints + 1) * slider_spacing)
            
            # Use add_mesh with text geometry for the reset button (alternative approach)
            try:
                plotter.add_checkbox_button_widget(
                    reset_all_sliders,
                    value=False,
                    position=(20, int((button_y - 0.02) * plotter.window_size[1])),  # Convert to pixel coordinates
                    size=80,
                    border_size=2,
                    color_on='lightgreen',
                    color_off='lightgray',
                ...
                plotter.add_text(
                    "Press 'r' key to reset sliders to zero",
                    position='upper_left',
                    font_size=10,
        ...
        try:
            ...
            try:
                ...
                plotter.screenshot("temp_urdf_view.png", return_img=False)
                print("Screenshot saved as 'temp_urdf_view.png'")
    ...
    def validate(self, validation_fn=None) -> bool:
        ...
        Args:
            validation_fn (function, optional): A function f(list[URDFError]) -> bool. None uses the strict handler (any error leads to False). Defaults to None.
    ...
    def split_along_joints(self, joint_type="floating", **kwargs):
        ...
        Returns:
            list[(np.ndarray, URDF)]: A list of tuples (np.ndarray, URDF) where each homogeneous 4x4 matrix describes the root transformation of the respective URDF model w.r.t. the original URDF.
        """
        root_urdf = URDF(
    ...
    def __eq__(self, other):
        if not isinstance(other, URDF):
            raise NotImplementedError("Equality comparison not implemented for this type.")
